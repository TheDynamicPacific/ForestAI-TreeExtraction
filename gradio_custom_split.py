...
# Custom HTML template for split map
SPLIT_MAP_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="map-container">
        ...
        <div class="comparison-container">
            <div class="comparison-left" id="left-map"></div>
            <div class="comparison-right" id="right-map"></div>
    ...
    <script>
        ...
        var satelliteLayer = L.tileLayer('{satellite_url}', {{
        ...
        // Add layers to left map (satellite only)
        satelliteLayer.addTo(leftMap);
        
        // Add layers to right map (GeoJSON only)
        baseLayer.addTo(rightMap);
        geojsonLayer.addTo(rightMap);
        
        // Sync maps
        map.on('move', function() {{
            leftMap.setView(map.getCenter(), map.getZoom());
            rightMap.setView(map.getCenter(), map.getZoom());
        }});
        
        leftMap.on('move', function() {{
            map.setView(leftMap.getCenter(), leftMap.getZoom());
            rightMap.setView(leftMap.getCenter(), leftMap.getZoom());
        }});
        
        rightMap.on('move', function() {{
            map.setView(rightMap.getCenter(), rightMap.getZoom());
        ...
        var leftSide = document.querySelector('.comparison-left');
        var rightSide = document.querySelector('.comparison-right');
        ...
        function handleDrag(e) {{
            ...
            rightSide.style.width = (100 - percent) + '%';
...
def create_custom_split_map(geojson_path, geotiff_path, feature_type):
    """Create a custom split map with the GeoJSON and GeoTIFF layers."""
    try:
        logger.info(f"Creating custom split map with {geojson_path} and {geotiff_path}")
        ...
        # Convert GeoTIFF to XYZ tiles URL
        # For simplicity, we'll use a placeholder URL and rely on the actual GeoTIFF being displayed in the main map
        satellite_url = "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        ...
        logger.info(f"Custom split map saved to {html_path}")
        
        return html_path
    except Exception as e:
        logger.error(f"Error creating custom split map: {str(e)}")
        logger.error(traceback.format_exc())
        return None
...
def upload_and_process(geotiff_file, feature_type):
    ...
    try:
        ...
        # Create a custom split map
        logger.info(f"Creating custom split map with {geojson_path} and {geotiff_path}")
        html_path = create_custom_split_map(geojson_path, geotiff_path, feature_type)
        
        if html_path:
            logger.info(f"Custom split map created at {html_path}")
            ...
            logger.error("Failed to create custom split map")
            return None, "Failed to create custom split map"
    except Exception as e:
...
def create_interface():
    ...
    with gr.Blocks(title="ForestAI - Feature Extraction with Split Map") as app:
        ...
        4. Use the slider in the middle of the map to compare the satellite imagery (left) with the extracted features (right)
